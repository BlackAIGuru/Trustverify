...
export class AuditService {
  static logEvent(event: AuditEvent, req?: Request) {
    ...
    if (event.riskLevel === 'critical') {
      this.logSecurityEvent(auditData);
    }
  }
  ...
  private static logSecurityEvent(auditData: any) {
...
AuditService.determineEventType = (req: Request, statusCode: number): AuditEventType | null => {
  const path = req.path;
