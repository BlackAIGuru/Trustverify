...
export default function VerificationPage() {
  ...
  // KYC status query
  const { data: kycStatus, isLoading: kycLoading, refetch } = useQuery({
    queryKey: ['/api/kyc/status'],
    queryFn: () => fetch('/api/kyc/status').then(res => res.json()),
    enabled: !!user
  });
  ...
  const onSubmit = async (data: VerificationForm) => {
    ...
    try {
      ...
      formData.append('address', `${data.address}, ${data.city}, ${data.country}, ${data.postalCode}`);
      
      if (frontIdFile) {
        formData.append('frontImage', frontIdFile);
      }
      if (backIdFile) {
        formData.append('backImage', backIdFile);
      }
      if (selfieFile) {
      ...
      // Submit to real API
      kycSubmissionMutation.mutate(formData);
    } catch (error) {
                  ...
                  <CardContent className="p-6">
                    ...
                    <div className="flex justify-between">
                      ...
                      <Button 
                        ...
                        disabled={!selfieFile || uploading || kycSubmissionMutation.isPending}
                        className="bg-green-600 hover:bg-green-700 text-white"
                      >
                        {uploading || kycSubmissionMutation.isPending ? "Submitting..." : "Submit Verification"}
              ...
              {step === 4 && (
                <CardContent className="p-8">
                    ...
                    <Card className="border-green-200 bg-green-50">
                      <CardContent className="p-6">
                        <div className="text-center">
                          ...
                          <p className="text-sm text-gray-600 mb-4">Excellent verification rating</p>
                    ...
                    <Card className="border-blue-200 bg-blue-50">
                      <CardContent className="p-6">
                        <div className="text-center">
                          ...
                          <h3 className="text-lg font-semibold text-gray-900 mb-2">Status: Verified</h3>
                          <p className="text-sm text-gray-600 mb-2">All checks passed successfully</p>
                  ...
                  <div className="bg-gray-50 rounded-lg p-6 mb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <p className="font-medium text-gray-900">Document Authentication</p>
                          <p className="text-sm text-gray-600">Valid government-issued ID verified</p>
                        </div>
                      </div>
                      <div className="flex items-center space-x-3">
                        <CheckCircle className="h-5 w-5 text-green-500 flex-shrink-0" />
                        <div>
                          <p className="font-medium text-gray-900">Biometric Matching</p>
                          <p className="text-sm text-gray-600">98.7% face-to-photo match confidence</p>
                        </div>
                      </div>
                      <div className="flex items-center space-x-3">
                        <CheckCircle className="h-5 w-5 text-green-500 flex-shrink-0" />
                        <div>
                          <p className="font-medium text-gray-900">Identity Cross-Check</p>
                          <p className="text-sm text-gray-600">No fraud indicators detected</p>
                        </div>
                      </div>
                      <div className="flex items-center space-x-3">
                        <CheckCircle className="h-5 w-5 text-green-500 flex-shrink-0" />
                        <div>
                          ...
                          <p className="text-sm text-gray-600">Clear - No watchlist matches</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-8">
                    <h3 className="font-medium text-blue-900 mb-3">Your Account Benefits</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="flex items-center space-x-2">
                        <CheckCircle className="h-4 w-4 text-green-500" />
                        <span className="text-sm text-blue-700">Transaction limits: Unlimited</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <CheckCircle className="h-4 w-4 text-green-500" />
                        <span className="text-sm text-blue-700">Trust score: 94/100 (Excellent)</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <CheckCircle className="h-4 w-4 text-green-500" />
                        <span className="text-sm text-blue-700">Verification level: Full</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <CheckCircle className="h-4 w-4 text-green-500" />
                        <span className="text-sm text-blue-700">Premium support access</span>
                      </div>
                    </div>
                  </div>
