...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/api-keys", requireAuth, async (req, res) => {
    try {
      ...
      const formattedKeys = apiKeys.map(key => ({
        ...key,
        key: (key.keyPrefix || 'tv_xxx_xxxxxx') + '_' + '•'.repeat(56), // Show prefix + hidden chars
  ...
  app.post("/api/api-keys", requireAuth, validateBody(z.object({
    ...
    try {
      ...
      const { name, industry, useCase, permissions } = req.body;
      ...
      // Generate API key
      const keyPrefix = `tv_${industry.substring(0, 3)}_${Math.random().toString(36).substring(2, 8)}`;
      ...
      res.json({
        ...apiKey,
        key: fullKey, // Only return full key once on creation
  ...
  app.post("/api/kyb/submit", requireAuth, upload.fields([
    ...
    try {
      ...
      // Company registration document is required (primary document)
      if (!files.companyRegistration?.[0]) {
        return res.status(400).json({ message: "Company registration document is required" });
      }

      // Store primary document URL (company registration) for backward compatibility
      const primaryDocumentUrl = files.companyRegistration[0].path;
      
      // Collect all document URLs for reference (stored in notes as JSON for now)
      const documentUrls: { [key: string]: string } = {
        companyRegistration: primaryDocumentUrl
      };
      
      if (files.boardOfDirectors?.[0]) {
        documentUrls.boardOfDirectors = files.boardOfDirectors[0].path;
      }
      if (files.businessLicense?.[0]) {
        documentUrls.businessLicense = files.businessLicense[0].path;
      }
      if (files.taxDocument?.[0]) {
        documentUrls.taxDocument = files.taxDocument[0].path;
      }
      if (files.otherDocument) {
        documentUrls.otherDocuments = files.otherDocument.map(f => f.path);
      }
      ...
      const kyb = await storage.createKybVerification({
        ...
        notes: JSON.stringify({ documentUrls, uploadedDocuments: Object.keys(documentUrls) })
      });
  ...
  // Payment Confirmation and Escrow Routes
  app.post("/api/payment/confirm", requireAuth, async (req, res) => {
    try {
      ...
      const confirmed = await escrowService.confirmPaymentIntent(parseInt(transactionId), paymentMethodId);

      res.json({
        message: "Payment confirmed successfully",
        escrow: confirmed,
        clientSecret: confirmed.id, // Payment intent ID for frontend
      });
    } catch (error: any) {
  ...
  app.post("/api/escrow/create", requireAuth, async (req, res) => {
    try {
      ...
      const { escrowService } = await import("./services/escrowService");
      const escrowAccount = await escrowService.createEscrowTransaction(parseInt(transactionId), providerPreference);

      // Get client secret for frontend payment confirmation (only for Stripe)
      let clientSecret = null;
      if (escrowAccount.providerId === 'stripe_treasury') {
        try {
          ...
          const paymentIntent = await stripe.paymentIntents.retrieve(escrowAccount.id);
          clientSecret = paymentIntent.client_secret;
        } catch (error) {
          console.warn('Could not retrieve Stripe client secret:', error);
        }
      }
      ...
      res.json({
        ...
        clientSecret, // Only available for Stripe, null for Escrow.com
      });
    } catch (error: any) {
  ...
  app.post("/api/escrow/release", requireAuth, async (req, res) => {
    try {
      ...
      const { escrowService } = await import("./services/escrowService");
      const releaseResult = await escrowService.releaseEscrowFunds(parseInt(transactionId), amount);
  ...
  app.post("/api/webhooks/escrow", express.raw({ type: 'application/json' }), async (req, res) => {
    try {
      const { config } = await import("./config");
      const webhookToken = config.ESCROW_COM_WEBHOOK_TOKEN;

      if (!webhookToken) {
        console.warn('Escrow.com webhook token not configured');
        return res.status(500).json({ error: 'Webhook not configured' });
      }

      // Verify webhook signature (Escrow.com uses HMAC SHA256)
      ...
      // Handle Escrow.com webhook events
      const transactionId = event.data?.transaction_id || event.transaction_id;
      
      if (!transactionId) {
        return res.status(400).json({ error: 'Transaction ID missing from webhook event' });
      }

      // Find transaction by escrow ID (stored in stripePaymentIntentId field)
      const transaction = await storage.getTransactionByEscrowId(transactionId);
      
      if (!transaction) {
        console.warn(`Transaction not found for Escrow.com transaction ID: ${transactionId}`);
        return res.status(404).json({ error: 'Transaction not found' });
      }

      switch (event.type) {
        case 'transaction.created':
          // Update transaction status to pending_acceptance
          await storage.updateTransactionStatus(transaction.id, 'pending_acceptance');
          await storage.updateTransactionStripeId(transaction.id, transactionId);
          break;

        case 'funds.received':
          // Update transaction status to funded
          await storage.updateTransactionStatus(transaction.id, 'escrow');
          await storage.updateTransactionStripeId(transaction.id, transactionId);
          break;
        ...
        default:
          console.log('Unhandled Escrow.com webhook event:', event.type);
      }

      res.status(200).json({ received: true });
    } catch (error: any) {
      console.error("Escrow.com webhook error:", error);
