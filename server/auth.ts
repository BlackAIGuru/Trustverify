...
export function setupAuth(app: Express) {
  ...
  const sessionSettings: session.SessionOptions = {
    ...
    name: 'trustverify.sid', // Explicit session name
    cookie: {
      secure: config.NODE_ENV === 'production', // Only secure in production with HTTPS
      sameSite: 'lax', // Use 'lax' for same-site requests (works with HTTPS)
      httpOnly: true,
      maxAge: 7 * 24 * 60 * 60 * 1000, // 7 days
      // Don't set domain explicitly - let browser handle it based on request origin
      // This ensures cookies work for both www.trustverify.co.uk and trustverify.co.uk
    },
  };
  ...
  // Fallback when OAuth is not configured: redirect to login with a message
  const oauthNotConfigured = (_req: express.Request, res: express.Response) => {
  ...
  app.get("/auth/facebook", oauthNotConfigured);
  app.get("/auth/facebook/callback", oauthNotConfigured);
  app.get("/auth/github", oauthNotConfigured);
  app.get("/auth/github/callback", oauthNotConfigured);
  app.get("/auth/apple", oauthNotConfigured);
  app.get("/auth/apple/callback", oauthNotConfigured);
    ...
    try {
      ...
      if (process.env.SENDGRID_API_KEY) {
        try {
          ...
          const msg = {
            to: email,
            from: 'noreply@trustverify.io', // Use your verified sender
